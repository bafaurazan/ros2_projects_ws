...
Pattern in every example: symptom → file that *looks* related → module that actually decides. Use these to score hypotheses, not as a canned architecture for other repos. In replies to the user say **hardware interface** and **driver core**, never unlabeled HI.
...
- Stan błędu po retry staje się prawdziwy gdy: hardware interface woła Activate ponownie, a driver core nadal ma error / stare ramki.
- Owner zmiany: **hardware interface** (retry / rollback lifecycle), plus ewentualny invariant w driver core — nie „cała aktywacja w driver core”.
...
**Example nudge after the five blocks (do not answer it in the same turn):** Kto nadal ustawia błąd albo kolejkuje ramkę po nieudanym `Activate` — otwórz to miejsce, nie komentarz przy `if`.
...
**What the author meant:** the branch is a gate. The design lives where the flag is **set** (who assigns it, from which thread/callback, after which event). Comments that say “if X then Y” skip that path.

**Skill must:** send the user to callers and to the store/`export`/assignment of the condition. Score “where this changes” on that module, not on the `if`.
...
**Wrong room:** a tidy change in the file they have open, not in the module that **sets** the state or decides retry.

**What the author meant:** a good-looking extract or extra `ClearAll` in driver core can still be the wrong layer if retry policy lives in the hardware interface. Quality questions are not a yes: same module as the writer? same event still required? if we drop the old condition, what still breaks?
