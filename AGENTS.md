...
- `importer transporter|notaura_ws` — clone that repo under `src/` (branch `develop`) on first use, then `load_macros`. No-op if already present.
...
- `notaura_ws_import_repos [docs|code|vendor|all|status]` — after `importer notaura_ws`.
