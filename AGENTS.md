...
- `git_ws <path> [path ...]` — recursively discover nested git repos under paths (always also checks `ros2_projects_ws` / `ROS2_PROJECTS_WS_ROOT`), fetch, report `ok`/`info`/`pull`/`push`/`merged`/`no upstream`/`manual` (`info` when fetch reported changes but the current branch is in sync; `merged` when no upstream and HEAD is already in `origin/develop`/`origin/main`, or local tracking remains after the remote branch was deleted — e.g. squash PR + delete), optional shared `y/N` for safe pull/push/switch(+pull).
