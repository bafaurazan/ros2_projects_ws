...
- `tr_pub [-y|--yes] [-clear] [path ...]` / `tr_sub [-y|--yes] [-clear]` — after `importer transporter`: `tr_pub` copies into `inbox/` as regular files (optional, one or more paths; nested `.git` is stripped so copies are not submodules), commits local changes, then pull+push commits ahead of the remote. `tr_sub` is fetch+pull and fails if the tree is dirty. `-clear` resets history (`tr_pub`: squash + force-push; `tr_sub`: `reset --hard` to remote). `-y` skips collision and `-clear` prompts.
