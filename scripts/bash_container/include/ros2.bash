...
container::_get_workspace_root_from_include() {
    # scripts/bash_container/include → workspace root
    cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd
}

container::_set_workspace_root() {
    export ROS2_PROJECTS_WS_ROOT="$(container::_get_workspace_root_from_include)"
}

container::_set_default_ros2_context() {
    export ROS_DISTRO="${ROS_DISTRO:-humble}"
    export ROS_DOMAIN_ID="${ROS_DOMAIN_ID:-0}"
}

container::_reset_overlay_prefix_paths() {
...
container::_get_ros2_setup_path() {
    printf '%s\n' "/opt/ros/${ROS_DISTRO}/local_setup.bash"
}

container::_has_ros2_setup() {
    [[ -f "$(container::_get_ros2_setup_path)" ]]
}

container::_source_ros2_setup_when_available() {
    container::_has_ros2_setup || return 0
    # shellcheck disable=SC1090
    source "$(container::_get_ros2_setup_path)"
}

# Drop overlay entries that point at removed build_ws/install_* trees.
container::_prune_prefix_path() {
    local var_name="$1"
...
container::_sanitize_overlay_paths() {
    container::_prune_prefix_path COLCON_PREFIX_PATH
    container::_prune_prefix_path AMENT_PREFIX_PATH
    container::_prune_prefix_path CMAKE_PREFIX_PATH
}

container::_is_broken_local_cmake() {
    local cmake_bin="$1"
    [[ "$cmake_bin" == "${HOME}/.local/bin/cmake" ]] || return 1
    ! "$cmake_bin" --version >/dev/null 2>&1
}

container::_has_system_cmake() {
    [[ -x /usr/bin/cmake ]]
}
...
container::_prefer_current_env_cmake() {
    container::_has_system_cmake || return 0

    export CMAKE_COMMAND=/usr/bin/cmake

    local cmake_bin
    cmake_bin="$(command -v cmake 2>/dev/null || true)"
    if container::_is_broken_local_cmake "$cmake_bin"; then
...
container::_get_cyclone_dds_config_path() {
...
container::_set_default_middleware() {
    export RMW_IMPLEMENTATION=rmw_cyclonedds_cpp
    export CYCLONEDDS_URI="file://$(container::_get_cyclone_dds_config_path)"
}
