...
_is_auto_local_display_enabled() {
    [[ "${ROS2_AUTO_LOCAL_DISPLAY:-1}" != "0" ]]
}

_has_x11_socket_for_display() {
...
_set_xauthority_from_local_candidates() {
    local cand
    shopt -s nullglob
    for cand in \
        "${HOME}/.Xauthority" \
...
_set_local_display_when_available() {
    ...
    _is_auto_local_display_enabled || return 0

    if ! _has_display; then
        for display_candidate in :0 :1; do
            if _has_x11_socket_for_display "$display_candidate"; then
                export DISPLAY="${display_candidate}"
                _has_xauthority || _set_xauthority_from_local_candidates || true
                return 0
            fi
        done
        return 0
    ...
    if ! _has_xauthority && _is_local_display; then
        _set_xauthority_from_local_candidates || true
    fi
}
