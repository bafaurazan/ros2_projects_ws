...
<repo>/scripts/macros/
  README.md
  api/                       # public API (sourced by load_macros)
    my_macro.bash
...
Small repos may keep a flat layout (`scripts/macros/*.bash` only); `load_macros` still supports that for compatibility.
...
### Root vs subrepo (`load.bash`)

This workspace root has an extra file that **must not** be copied into a subrepo:

- [`api/load.bash`](api/load.bash) — bootstrap: find every `scripts/macros/` bundle, check name collisions, `source` public APIs.
- Subrepos add only their own command files under `api/` (e.g. `notaura_setup.bash`).
...
`load_macros` sources `api/*.bash` (or top-level `*.bash` for flat bundles) from the original paths. Helpers are pulled in by those files.
...
- `load_macros` — rediscover `scripts/macros/` under the workspace root, validate names, source public APIs in place.

Entry: `scripts/macros/api/load.bash` (used by `session/macros/setup.bash` and `session/container/setup.bash`).
