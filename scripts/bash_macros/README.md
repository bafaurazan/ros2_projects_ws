...
- `git_ws <path> [path ...]` — recursively discover git repos under the given paths (skips `build` / `build_ws` / `install` / `log` / `trash`), always also checks the `ros2_projects_ws` repo (`ROS2_PROJECTS_WS_ROOT`), `git fetch --prune`, report `ok` / `info` / `pull` / `push` / `merged` / `set-upstream` / `push-upstream` / `no upstream` / `manual` (`info` = current branch in sync but fetch reported changes such as new remote branches; `merged` = no upstream and HEAD is already in `origin/develop`/`origin/main`, or tracking config remains after the remote branch was deleted — e.g. squash PR + delete; `set-upstream` = remote branch exists, set tracking with `git branch -u`; `push-upstream` = publish local branch with `git push -u`; on `develop`/`main` with `ok`/`info`, indented report-only `hint:` lines list `origin/*` tips ahead of integration — switch to a feature branch manually; fetch lines printed indented), then one `y/N` to apply safe `pull --ff-only` / `push` / `git switch` + `pull --ff-only` / `branch -u` / `push -u` when any exist.
