...
- `git_ws <path> [path ...]` — recursively discover git repos under the given paths (skips `build` / `build_ws` / `install` / `log` / `trash`), always also checks the `ros2_projects_ws` repo (`ROS2_PROJECTS_WS_ROOT`), `git fetch --prune`, then a diag-style report per repo: current branch, ahead/behind vs `origin/develop`, upstream sync, remote branches vs develop, and `local without remote`. Tags: `ok` / `info` / `pull` / `push` / `push-upstream` / `switch` / `manual` (`info` = in sync but fetch printed news; `switch` = no remote and HEAD already in `origin/develop`; `manual` = dirty, diverged, no develop, missing tracking when remote exists, etc.). Each safe action gets its own `y/N`; orphan locals get `p`/`d`/`N` with an on-screen legend (`d` only if the branch is merged into develop; never force-delete). No CLI flags; integration base is `develop` only (not `main`); does not run `git branch -u`.
