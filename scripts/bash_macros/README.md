...
- `git_ws <path> [path ...]` — recursively discover git repos under the given paths (skips `build` / `build_ws` / `install` / `log` / `trash`), always also checks the `ros2_projects_ws` repo (`ROS2_PROJECTS_WS_ROOT`), `git fetch --prune`, then a diag-style report per repo: current branch, ahead/behind vs `origin/develop`, upstream sync, remote branches vs develop, and `local without remote`. Tags: `ok` / `info` / `behind-develop` / `pull` / `push` / `push-upstream` / `switch` / `manual` (`info` = in sync but fetch printed news; `behind-develop` = sync with upstream but HEAD behind `origin/develop`; `switch` = no remote and HEAD already in `origin/develop`; `manual` = dirty, diverged, no develop, missing tracking when remote exists, etc.). Safe actions use per-prompt `y/N` (`pull` is `y`/`i`/`N`: `i` shows incoming `git log` + `git diff` vs upstream); orphan locals get `p`/`d`/`N` always (`d` deletes local only after a second `Are you sure...? [y/N]`; unmerged / squash use `branch -D`). Ends with a `summary` of repo basename → `[tag]` (re-assessed after a successful apply). No CLI flags; integration base is `develop` only (not `main`); does not run `git branch -u`.
